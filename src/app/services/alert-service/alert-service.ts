  ...
  discordNotification(title: string, message: string): void {
    ...
    notification.innerHTML = `
      <div class="flex items-start space-x-3">
        <div class="w-6 h-6 bg-nimbus-blue rounded-full flex items-center justify-center flex-shrink-0">
          <svg class="w-4 h-4 text-white" fill="currentColor" viewBox="0 0 20 20">
            <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"/>
          </svg>
        </div>
        <div class="flex-1 min-w-0">
          <h4 class="text-nimbus-text-lighter font-semibold text-sm mb-1">${title}</h4>
          <p class="text-nimbus-text text-sm">${message}</p>
        </div>
        <button class="text-nimbus-text-muted hover:text-nimbus-text-light transition-colors duration-150" onclick="this.parentElement.parentElement.remove()">
